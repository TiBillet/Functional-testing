...
test.describe('Envoyer en préparation, paiemant cashless, fonds insuffisants sur une carte.', () => {
  ...
  test("Retour pour la 2ème carte (fonds suffisants).", async () => {
    ...
    await expect(page.locator('#popup-cashless #test-total-carte-avant-achats')).toHaveText('0.00')

    // reste sur carte
    await expect(page.locator('#popup-cashless .test-carte-apres-achats')).toHaveText('Reste sur carte : 0.00')
